...
[[ "$(git rev-parse HEAD)" == "$GITHUB_SHA" && "${PUBLISH_BETA12:-}" =~ ^(true|false)$ ]]
...
api repos/RTBGG/Stackfort/actions/runs/36005061649 >"$workspace/build.json"
artifacts 36005061649 >"$workspace/build-artifacts.json"
...
api repos/RTBGG/Stackfort/actions/runs/36007265703 >"$workspace/tag-run.json"
artifacts 36007265703 >"$workspace/tag-artifacts.json"
node scripts/verify-beta12-fresh-scope.mjs "$workspace/promotion.json" \
  docs/release-evidence/2026-09-24-beta12-fresh-install-scope.md \
  "$workspace/tag-run.json" "$workspace/tag-artifacts.json" >"$workspace/upgrade-support.json"
api repos/RTBGG/Stackfort/actions/artifacts/10811230131/zip >"$workspace/original.zip"
python3 scripts/extract-release-candidate.py "$workspace/promotion.json" "$workspace/original.zip" "$workspace/original"
api repos/RTBGG/Stackfort/actions/artifacts/10810852574/zip >"$workspace/tag.zip"
...
[[ "$(sha256sum dist/build-attestation.jsonl | cut -d ' ' -f 1)" == 67307840e6be9d9ff659fa22d6cb965098f4edc3742b4929a956448ef0659555 ]]
...
printf 'All unchanged technical gates passed for retained Beta.12. Upgrade support is explicitly absent.\n'
if [[ "$PUBLISH_BETA12" != true ]]; then
...
fi
{
  ...
  printf '\nThe original packages, source tag and tag-origin attestation are unchanged. This release fixes rejection of inactive optional optical-media fstab entries; it does not repair already shipped predecessor updaters.\n'
} >"$workspace/notes.md"
gh release create "$tag" dist/* --repo RTBGG/Stackfort --verify-tag --prerelease --latest=false \
  --title 'Stackfort v0.1.0-beta.12 — experimental, fresh installs only' --notes-file "$workspace/notes.md"
