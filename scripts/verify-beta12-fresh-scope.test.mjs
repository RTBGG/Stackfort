...
import { beta12Candidate, tagQualification, verifyFreshScope } from './verify-beta12-fresh-scope.mjs';

function fixture() {
  return {
    candidate: structuredClone(beta12Candidate),
    report: readFileSync(new URL('../docs/release-evidence/2026-09-24-beta12-fresh-install-scope.md', import.meta.url)),
    run: { id: 36007265703, run_attempt: 1, head_sha: beta12Candidate.commit, path: '.github/workflows/release.yml', event: 'push', head_branch: 'v0.1.0-beta.12', status: 'completed', conclusion: 'failure', repository: { full_name: 'RTBGG/Stackfort' }, head_repository: { full_name: 'RTBGG/Stackfort' } },
    artifacts: { total_count: 1, artifacts: [{ id: 10810852574, name: 'stackfort-tag-candidate-0.1.0-beta.12-attempt-1', expired: false, size_in_bytes: 313208244, digest: 'sha256:' + tagQualification.artifactSHA256, workflow_run: { id: 36007265703, head_sha: beta12Candidate.commit } }] },
  };
}
...
test('different candidate, scope, source, attempt or artifact fail closed', () => {
  const changes = [
    v => { v.candidate.version = '0.1.0-beta.13'; },
