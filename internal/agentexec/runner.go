...
const (
	...
	ProfileSystemdShowAccountUserManager    ProfileID = "resource.systemd-show-account-user-manager"
	ProfileSystemdRestartAccountUserManager ProfileID = "resource.systemd-restart-account-user-manager"
...
func NewRunner() *Runner {
	...
	return &Runner{profiles: map[ProfileID]executionProfile{
		...
		ProfileSystemdShowAccountUserManager: accountResourceProfile(func(spec hostingresources.Spec) []string {
			unit, _ := hostingresources.UserManagerUnitName(spec.Identity.UID)
			return []string{
				"show", "--no-pager", "--property=ActiveState", "--property=ControlGroup", unit,
			}
		}),
		ProfileSystemdRestartAccountUserManager: accountResourceProfile(func(spec hostingresources.Spec) []string {
			unit, _ := hostingresources.UserManagerUnitName(spec.Identity.UID)
