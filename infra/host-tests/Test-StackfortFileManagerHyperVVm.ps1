...
$testBinary = Join-Path $workRoot 'stackfort-file-manager.test'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "File-manager integration build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $remoteBinary = '/tmp/stackfort-file-manager.test'
    & scp.exe @sshOptions $testBinary "${GuestUser}@${address}:${remoteBinary}"
    if ($LASTEXITCODE -ne 0) { throw "Copying the file-manager test failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteBinary' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 '$remoteBinary' -test.v -test.run '^TestDisposableHostFileManagerNavigation$' -test.timeout=4m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "File-manager qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION file-manager-navigation=passed')) {
        throw 'File-manager qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; FileManagerNavigation = 'passed' }
} finally {
