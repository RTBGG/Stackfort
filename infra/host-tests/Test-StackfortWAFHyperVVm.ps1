...
$testBinary = Join-Path $workRoot 'stackfort-waf.test'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "WAF integration build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $packageCheck = if ($ImageId -eq 'rocky-10') {
        'sudo rpm -q stackfort-waf >/dev/null && sudo test "$(getenforce)" = Enforcing && sudo ls -Zd /var/cache/stackfort/coraza | grep -q httpd_cache_t'
    } else {
        'sudo dpkg-query -W stackfort-waf >/dev/null && grep -q "^Y" /sys/module/apparmor/parameters/enabled'
    }
    & ssh.exe @sshOptions "${GuestUser}@${address}" $packageCheck
    if ($LASTEXITCODE -ne 0) { throw "Installed WAF/security prerequisite check failed for $VmName." }

    $remoteBinary = '/tmp/stackfort-waf.test'
    & scp.exe @sshOptions $testBinary "${GuestUser}@${address}:${remoteBinary}"
    if ($LASTEXITCODE -ne 0) { throw "Copying the WAF test failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteBinary' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 '$remoteBinary' -test.v -test.run '^TestDisposableHostWAFRuntimeAndPerformance$' -test.timeout=12m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "WAF qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    foreach ($marker in @(
        'STACKFORT_QUALIFICATION waf-reload-failure-rollback=passed',
        'STACKFORT_QUALIFICATION waf-acme-bypass=passed',
        'STACKFORT_QUALIFICATION waf-tls=passed',
        'STACKFORT_QUALIFICATION waf-mandatory-access-control=passed',
        'STACKFORT_QUALIFICATION waf-runtime-attack-corpus=passed',
        '"name":"waf-static-off"',
        '"name":"waf-static-detection"',
        '"name":"waf-static-blocking-pl1"',
        '"name":"waf-static-mode-overhead"'
    )) {
        if (-not $text.Contains($marker)) { throw "WAF qualification marker is missing: $marker" }
    }
    [pscustomobject]@{
        VMName = $VmName
        ImageId = $ImageId
        AttackCorpus = 'passed'
        ReloadRollback = 'passed'
        ACMEBypass = 'passed'
        TLS = 'passed'
