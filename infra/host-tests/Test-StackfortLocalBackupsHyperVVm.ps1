...
$testBinary = Join-Path $workRoot 'stackfort-local-backups.test'
$agentBinary = Join-Path $workRoot 'stackfort-agent-local-backups'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "Local-backup integration build failed with exit code $LASTEXITCODE." }
            & go.exe build -o $agentBinary ./cmd/stackfort-agent
            if ($LASTEXITCODE -ne 0) { throw "Local-backup agent build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $remoteTest = '/tmp/stackfort-local-backups.test'
    $remoteAgent = '/tmp/stackfort-agent-local-backups'
    & scp.exe @sshOptions $testBinary $agentBinary "${GuestUser}@${address}:/tmp/"
    if ($LASTEXITCODE -ne 0) { throw "Copying local-backup binaries failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteTest' '$remoteAgent' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 STACKFORT_AGENT_HELPER='$remoteAgent' '$remoteTest' -test.v -test.run '^TestDisposableHostLocalBackupRestore$' -test.timeout=12m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "Local-backup qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION local-backup-restore=passed')) {
        throw 'Local-backup qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; LocalBackupRestore = 'passed' }
} finally {
