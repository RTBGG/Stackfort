...
$testBinary = Join-Path $workRoot 'stackfort-scheduled-jobs.test'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "Scheduled-job integration build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $packageCommand = switch ($ImageId) {
        'debian-13' { 'if ! command -v php8.4 >/dev/null 2>&1; then sudo env DEBIAN_FRONTEND=noninteractive apt-get update -qq && sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y -qq php8.4-cli; fi' }
        'ubuntu-26.04' { 'if ! command -v php8.5 >/dev/null 2>&1; then sudo env DEBIAN_FRONTEND=noninteractive apt-get update -qq && sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y -qq php8.5-cli; fi' }
        'rocky-10' { 'if ! command -v php >/dev/null 2>&1; then sudo dnf install -y -q php-cli; fi' }
    }
    & ssh.exe @sshOptions "${GuestUser}@${address}" $packageCommand
    if ($LASTEXITCODE -ne 0) { throw "Preparing the scheduled-job runtime failed with exit code $LASTEXITCODE." }

    $remoteBinary = '/tmp/stackfort-scheduled-jobs.test'
    & scp.exe @sshOptions $testBinary "${GuestUser}@${address}:${remoteBinary}"
    if ($LASTEXITCODE -ne 0) { throw "Copying the scheduled-job test failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteBinary' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 '$remoteBinary' -test.v -test.run '^TestDisposableHostScheduledJobs$' -test.timeout=8m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "Scheduled-job qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION scheduled-jobs=passed')) {
        throw 'Scheduled-job qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; ScheduledJobs = 'passed' }
} finally {
