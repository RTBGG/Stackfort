...
$testBinary = Join-Path $workRoot 'stackfort-file-archives.test'
$agentBinary = Join-Path $workRoot 'stackfort-agent-file-archives'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "File-archive integration build failed with exit code $LASTEXITCODE." }
            & go.exe build -o $agentBinary ./cmd/stackfort-agent
            if ($LASTEXITCODE -ne 0) { throw "File-archive agent build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $remoteTest = '/tmp/stackfort-file-archives.test'
    $remoteAgent = '/tmp/stackfort-agent-file-archives'
    & scp.exe @sshOptions $testBinary $agentBinary "${GuestUser}@${address}:/tmp/"
    if ($LASTEXITCODE -ne 0) { throw "Copying file-archive binaries failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteTest' '$remoteAgent' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 STACKFORT_AGENT_HELPER='$remoteAgent' '$remoteTest' -test.v -test.run '^TestDisposableHostFileArchiveOperations$' -test.timeout=10m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "File-archive qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION file-manager-archives=passed')) {
        throw 'File-archive qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; FileManagerArchives = 'passed' }
} finally {
