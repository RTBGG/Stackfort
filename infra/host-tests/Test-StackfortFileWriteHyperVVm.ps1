...
$testBinary = Join-Path $workRoot 'stackfort-file-write.test'
$agentBinary = Join-Path $workRoot 'stackfort-agent-file-write'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "File-write integration build failed with exit code $LASTEXITCODE." }
            & go.exe build -o $agentBinary ./cmd/stackfort-agent
            if ($LASTEXITCODE -ne 0) { throw "File-write agent build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $remoteTest = '/tmp/stackfort-file-write.test'
    $remoteAgent = '/tmp/stackfort-agent-file-write'
    & scp.exe @sshOptions $testBinary $agentBinary "${GuestUser}@${address}:/tmp/"
    if ($LASTEXITCODE -ne 0) { throw "Copying file-write binaries failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteTest' '$remoteAgent' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 STACKFORT_AGENT_HELPER='$remoteAgent' '$remoteTest' -test.v -test.run '^TestDisposableHostFileManagerWrite$' -test.timeout=4m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "File-write qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION file-manager-write=passed')) {
        throw 'File-write qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; FileManagerWrite = 'passed' }
} finally {
