...
# BIOS/MBR lab only: current installer with retained beta.3 laboratory payload.
# This is not qualification or publication of a new release archive.
# Fixed disposable VM only; checkpoint restoration is always a separate action.
[CmdletBinding()]
param([ValidateSet('Prepare','Arm','Validate','NormalBoot','Rejected','RejectedCurrent','ValidateCurrent','StorageAfterPackageFailure','NormalBootAfterPackageFailure')][string] $Stage='Validate', [switch] $AcceptDisposableReinstallationRisk)
...
$taskVMName='stackfort-native-mbr-debian-13'
$taskVM=Get-VM -Name $taskVMName
if ($taskVM.Id -ne '1f5e665a-fddd-4cd2-bc55-44255b01963d') { throw 'Unexpected VM identity' }
...
if ($taskMemory.DynamicMemoryEnabled -or $taskMemory.Startup -lt 4GB) { throw 'MBR qualification requires fixed RAM of at least 4 GiB, including after reboot.' }
if ((Get-VM -Name 'stackfort-native-restore-rescue').State -ne 'Off') { throw 'The duplicate-identity restored clone must remain off' }
$taskKnownHosts=Join-Path $PSScriptRoot 'work/mbr-lab-20260924/known_hosts'
$taskSSH=@('-o','BatchMode=yes','-o','IdentitiesOnly=yes','-o','ConnectTimeout=5','-o','StrictHostKeyChecking=yes','-o',"UserKnownHostsFile=$taskKnownHosts",'-o',"HostKeyAlias=$taskVMName",'-i','C:\ProgramData\Stackfort\Hyper-V\keys\stackfort-host-test-ed25519')
...
$taskRun=New-Item -ItemType Directory -Path (Join-Path $PSScriptRoot ('work/mbr-native-boot-'+$Stage+'-'+[DateTime]::UtcNow.ToString('yyyyMMddTHHmmssZ')))
...
if ($Stage -in @('Validate','NormalBoot','Rejected','NormalBootAfterPackageFailure')) {
...
$selection=@{Prepare='TestDisposableNativeBootPrepare';Arm='TestDisposableNativeBootArm';Validate='TestDisposableNativeBootValidate';NormalBoot='TestDisposableNativeBootValidate';ValidateCurrent='TestDisposableNativeBootValidate';Rejected='TestDisposableNativeBootRejected';RejectedCurrent='TestDisposableNativeBootRejected';StorageAfterPackageFailure='TestDisposableNativeMBRStorageAfterPackageFailure';NormalBootAfterPackageFailure='TestDisposableNativeMBRStorageAfterPackageFailure'}[$Stage]
$normal=if ($Stage -in @('NormalBoot','NormalBootAfterPackageFailure')) {'1'} else {'0'}
$recovery=if ($Stage -eq 'Prepare' -and $AcceptDisposableReinstallationRisk) {'1'} else {'0'}
& ssh.exe @taskSSH "stackfort-test@$address" "chmod 0755 /tmp/native-boot.test && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 STACKFORT_NATIVE_QUOTA_PROTOTYPE=1 STACKFORT_NATIVE_DISPOSABLE_RECOVERY_ACCEPTED=$recovery STACKFORT_NATIVE_JOURNAL_NORMAL_BOOT=$normal /tmp/native-boot.test -test.v -test.timeout=10m -test.run='^$selection`$'" | Tee-Object -FilePath (Join-Path $taskRun.FullName 'tests.log')
