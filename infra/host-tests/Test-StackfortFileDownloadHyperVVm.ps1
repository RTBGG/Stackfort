...
$testBinary = Join-Path $workRoot 'stackfort-file-download.test'
$agentBinary = Join-Path $workRoot 'stackfort-agent-file-download'
if (-not $SkipBuild) {
    ...
    try {
        ...
        try {
            ...
            if ($LASTEXITCODE -ne 0) { throw "File-download integration build failed with exit code $LASTEXITCODE." }
            & go.exe build -o $agentBinary ./cmd/stackfort-agent
            if ($LASTEXITCODE -ne 0) { throw "File-download agent build failed with exit code $LASTEXITCODE." }
...
try {
    ...
    $remoteTest = '/tmp/stackfort-file-download.test'
    $remoteAgent = '/tmp/stackfort-agent-file-download'
    & scp.exe @sshOptions $testBinary $agentBinary "${GuestUser}@${address}:/tmp/"
    if ($LASTEXITCODE -ne 0) { throw "Copying file-download binaries failed with exit code $LASTEXITCODE." }
    $output = & ssh.exe @sshOptions "${GuestUser}@${address}" `
        "chmod 0755 '$remoteTest' '$remoteAgent' && sudo env STACKFORT_DISPOSABLE_HOST_TEST=1 STACKFORT_AGENT_HELPER='$remoteAgent' '$remoteTest' -test.v -test.run '^TestDisposableHostFileManagerDownload$' -test.timeout=4m" 2>&1
    $exitCode = $LASTEXITCODE
    $output | Write-Output
    if ($exitCode -ne 0) { throw "File-download qualification failed with exit code $exitCode." }
    $text = $output -join "`n"
    if (-not $text.Contains('STACKFORT_QUALIFICATION file-manager-download=passed')) {
        throw 'File-download qualification marker is missing.'
    }
    [pscustomobject]@{ VMName = $VmName; ImageId = $ImageId; FileManagerDownload = 'passed' }
} finally {
